...
from ml4audio.speaker_tasks.speaker_clusterer import (
    UmascanSpeakerClusterer,
    CalibrationDatum,
)
...
@pytest.mark.skip("why did I skip this? probably just too lazy to fix it!")
def test_speaker_clusterer_oracle_vad(
    ...
    clusterer: UmascanSpeakerClusterer = UmascanSpeakerClusterer(
        embedder=NemoAudioEmbedder(model_name="ecapa_tdnn"), metric="cosine"
    ).build()
